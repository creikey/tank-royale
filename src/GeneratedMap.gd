...
func generate_map():
	...
	for id in LobbySingleton.player_info.keys(): # because running on the server this includes everybody
		...
		id_to_spawnpoint[id] = Vector2(row, column)*maze_wall_length + Vector2(maze_wall_length, maze_wall_length)/2.0
...
func spawn_player(peer_id: int, info: Dictionary, is_my_player: bool, spawnpoint: Vector2):
...
remotesync func spawn_players():
	...
	if my_id != 1:
		spawn_player(my_id, LobbySingleton.my_info, true, id_to_spawnpoint[my_id])
...
remotesync func add_maze_bodies():
	for row in cell_data.size():
		for column in cell_data[row].size():
			...
			cur_maze_cell_body.position = Vector2(column, row)*cur_maze_cell_body.wall_length
